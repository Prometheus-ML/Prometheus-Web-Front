...
const formData = ref(_.cloneDeep(props.project))
...
const addMember = async () => {
  ...
  try {
    const response = await $api(`${import.meta.env.VITE_API_URL}/project/create_project`, {
      method: 'POST',
			body: formData.value,
    });
...
const updateMember = async (id) => {
		try {
			const response = await $api(`${import.meta.env.VITE_API_URL}/project/update/${id}`, {
				method: 'PUT',
				body: formData.value
			});
      for(let i = 0;i<imageStack.value.length;++i){
    ...
    // Use the project id to send the update request
    // You can access edited properties with project.editedName, project.editedEducation, etc.
    // Make the API request to update the project with the edited data
