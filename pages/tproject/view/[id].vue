<template>
  <div v-if="user" class="container mx-auto pt-16">
    ...
    <div v-if="user" class="flex justify-end gap-x-2 mb-5">
      <nuxt-link :to="'/tproject'" class="bg-white py-2 px-4 border rounded inline-block">
          글목록
      </nuxt-link>
      ...
      <nuxt-link :to="'/tproject/edit/'+project?.id" v-if="user?.id == project?.writer.id || user?.grant == 'admin'" class="bg-white py-2 px-4 border rounded inline-block">
          수정
      </nuxt-link>
    </div>
  </div>
...
onMounted(async () => {
	project.value = projects.find(project => project.id === parseInt(id));
